...
namespace s21 {
    ...
    //  -------------------------------Helper---------------------------- // НУЖНО БУДЕТ УДАЛИТЬ ВСЕ ПРИНТЫ !!!!!!!!!!!!!!!!

    void Model_3DViewer::PrintVertexArray() {
        ...
        for (size_t i = 0; i < VertexArray_.size(); i++) {
            outFile << "v " << VertexArray_[i].x << " " << VertexArray_[i].y << " " << VertexArray_[i].z << std::endl;
        }
        outFile.close();
        std::cout << CountVertex_ << std::endl;
    }

    void Model_3DViewer::PrintFaceArray() {
        std::string filename = "face.txt";
        std::ofstream outFile(filename);
        if (!outFile.is_open()) {
            std::cerr << "Error opening file: " << filename << std::endl;
            return;
        }
        for (size_t i = 0; i < FaceArray_.size(); i++) {
            outFile << "f ";
            for (size_t j = 0; j < FaceArray_[i].size() ; j++) {
                outFile << FaceArray_[i][j] << " ";
            }
        outFile << " count: " << FaceArray_[i].size() << std::endl;
        }
        outFile.close();
        // std::cout << CountFace_ << std::endl;
    }

    void Model_3DViewer::PrintVertexFromFace() {
         std::string filename = "vertex_face.txt";
        std::ofstream outFile(filename);
        if (!outFile.is_open()) {
            std::cerr << "Error opening file: " << filename << std::endl;
            return;
        ...
        for (size_t i = 0; i < FaceArray_.size(); i++) {
            ...
            for (size_t j = 0; j < FaceArray_[i].size() ; j++) {
                VertexCoordinates tempVertex = GetVertexFromFace(FaceArray_[i][j]);
                outFile << tempVertex.x << " " << tempVertex.y << " " << tempVertex.z << std::endl;
            }
        }
        outFile.close();
    }
